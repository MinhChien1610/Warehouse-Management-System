class DanhMuc:
    def kiem_tra_danh_muc_truoc_luu(self, data, danh_muc, truong_ma, ma_bo_qua, du_lieu):
        ...
        if danh_muc in cau_hinh_ten:
            ...
            if str(du_lieu.get(truong_ten, "")).strip() == "":
        ...
        if danh_muc in ["nhaSanXuat", "khachHang", "kho"]:
    ...
    def tao_thong_bao_dang_duoc_su_dung(self, ten_doi_tuong, ma, danh_sach_lien_quan):
        return "Không thể xóa " + ten_doi_tuong + " " + ma + " vì đang được sử dụng: " + ", ".join(danh_sach_lien_quan) + "."
    ...
    def xoa_loai_hang(self, ma_loai_hang):
        ...
        if so_san_pham > 0:
            raise ValueError(self.tao_thong_bao_dang_duoc_su_dung("loại hàng", ma_loai_hang, [str(so_san_pham) + " sản phẩm"]))
    ...
    def xoa_nha_san_xuat(self, ma_nha_san_xuat):
        ...
        if so_phieu_nhap > 0:
            raise ValueError(self.tao_thong_bao_dang_duoc_su_dung("nhà sản xuất", ma_nha_san_xuat, [str(so_phieu_nhap) + " phiếu nhập"]))
